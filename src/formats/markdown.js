...
function formatProject(result) {
    return [
        ...
        "This repository has an average cyclomatic complexity of **",
        result.moduleAverage.methodAverage.cyclomatic,
        "** ",
        "and a maintainability index of **",
        result.moduleAverage.maintainability,
        "**.\n",
        "Each function has an average of ",
        result.moduleAverage.methodAverage.sloc.logical,
        " lines of code and ",
        result.moduleAverage.methodAverage.paramCount,
        " parameters.\n",
        ...
        "\n",
        "* Bugs: ",
        result.moduleAverage.methodAverage.halstead.bugs,
        "\n",
        "* Difficulty: ",
        result.moduleAverage.methodAverage.halstead.difficulty,
        "\n",
        "* Time: ",
        result.moduleAverage.methodAverage.halstead.time,
        "\n",
        "* Volume: ",
        result.moduleAverage.methodAverage.halstead.volume,
        "\n"
    ].join("");
}
...
function createTableForFunctions(anArrayOfFunctionReports) {
    return (
        "|name|complexity|path|line|\n| --- | --- | --- | --- |\n" +
        anArrayOfFunctionReports
            .map(
                r =>
                    `|${r.name.replace(/<|>/g, "")}|${r.cyclomatic}|${
                        r.srcPath
                    }|${r.lineStart}|`
            )
            .join("\n")
...
function createTableForFunctionLengths(anArrayOfFunctionReports) {
    return (
        "|name|length|path|line|\n| --- | --- | --- | --- |\n" +
        anArrayOfFunctionReports
            .map(
                r =>
                    `|${r.name.replace(/<|>/g, "")}|${r.lineEnd -
                        r.lineStart}|${r.srcPath}|${r.lineStart}|`
            )
            .join("\n")
    );
}
...
function mostComplexFunctions(aComplexityReport, size = 10) {
    return getSortedFunctionReports(
        aComplexityReport,
...
function aggregateFunctionReports(aComplexityReport) {
    const result = [];
    aComplexityReport.modules.forEach(m =>
        m.methods.forEach(me => {
